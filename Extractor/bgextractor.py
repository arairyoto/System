...
class BackwardWordNetExtractor:
    ...
    def load_frequency(self):
        file_name = '%s/frequency.txt' % self._root
        f = codecs.open(file_name, 'r', 'utf-8')
        self.frequency = {}
        for line in f.readlines():
            sl = line.strip().split(' ')
            self.frequency[sl[0]] = float(sl[1])
            ...
            for word in wn.all_words(pos=pos, lang=self.lang):
                ...
                for synset in word.synsets():
                    ...
                    if synsetId in self.model.keys():
                        ...
                        if lexemeId in self.frequency.keys():
                            count = self.frequency[lexemeId]
                        else:
                            count = 0

                        fWords.write('%s,' % lexemeId)
                        fLexemes.write('%d %d %f\n' % (self.SynsetIndex[synsetId], wordCounterAll, count))
